...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (!bucketListItem)
      throw new Error(
        "Something went wrong while saving the bucket list items..."
      );
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!response) throw Error("Something went wrong with put");
    const updated = { ...response.__doc, ...req.body };
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!removed) throw Error("Something went wrong with delete");
